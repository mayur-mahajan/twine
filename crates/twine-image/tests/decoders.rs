...
mod decoders {
    ...
    fn encode_with_image(rgba: &[u8], w: u32, h: u32, fmt: image::ImageFormat, color: image::ColorType) -> Vec<u8> {
    ...
    fn qoi_decode_reference_images() {
        for (w, h, alpha) in [(64, 48, false), (37, 29, true), (1, 1, true), (300, 2, false)] {
            ...
            let color = if alpha { image::ColorType::Rgba8 } else { image::ColorType::Rgb8 };
            ...
            assert_eq!(hd.format, if alpha { ColorFormat::Argb8888 } else { ColorFormat::Xrgb8888 });
    ...
    fn qoi_rejects_bad_magic_and_huge_dims() {
        ...
        bad[0] = b'Q';
        assert_eq!(QoiDecoder.decode(&bad, &mut Vec::new()), Err(Error::InvalidHeader));
        let mut huge = good.clone();
        huge[4..8].copy_from_slice(&20_000u32.to_be_bytes());
        assert_eq!(QoiDecoder.decode(&huge, &mut Vec::new()), Err(Error::InvalidHeader));
        ...
        assert_eq!(QoiDecoder.decode(&long[..40], &mut Vec::new()), Err(Error::Decode("qoi truncated")));
    }

    // ------------------------------------------------------------------ BMP

    fn bmp24() -> Vec<u8> {
        encode_with_image(&photo(21, 13, false), 21, 13, image::ImageFormat::Bmp, image::ColorType::Rgb8)
    }

    fn bmp8() -> Vec<u8> {
        encode_with_image(&photo(21, 13, false), 21, 13, image::ImageFormat::Bmp, image::ColorType::L8)
    }
    ...
    fn bmp_decodes_24_and_8bit() {
        ...
        let gray = image::DynamicImage::ImageRgba8(image::RgbaImage::from_raw(21, 13, src).unwrap()).to_luma8();
        let expected: Vec<u8> = gray.pixels().flat_map(|p| [p.0[0], p.0[0], p.0[0], 255]).collect();
    ...
    fn jpeg_decodes_baseline_420_444() {
        ...
        for (sampling, progressive) in [(S::R_4_2_0, false), (S::R_4_4_4, false), (S::R_4_2_2, false), (S::R_4_2_0, true)] {
            let f = jpeg_file(sampling, progressive);
            let mut out = Vec::new();
            let h = JpegDecoder.decode(&f, &mut out).unwrap_or_else(|e| panic!("{sampling:?} {progressive}: {e}"));
            let px = out;
            assert_eq!((h.w, h.h, h.format), (48, 40, ColorFormat::Xrgb8888));
            let r = reference(&f);
            let worst = px.iter().zip(&r).map(|(a, b)| a.abs_diff(*b)).max().unwrap();
            assert!(worst <= 2, "{sampling:?} progressive={progressive}: max delta {worst}");
            // Close to the source as well (lossy, quality 90).
            let src = bgra(&photo(48, 40, false));
            let mean: u64 = px.iter().zip(&src).map(|(a, b)| u64::from(a.abs_diff(*b))).sum::<u64>() / px.len() as u64;
            assert!(mean <= 6, "{sampling:?}: mean error {mean}");
        }
        // Grayscale JPEG (from the `image` crate).
        let g = encode_with_image(&photo(30, 20, false), 30, 20, image::ImageFormat::Jpeg, image::ColorType::L8);
        let (h, px) = decode(&JpegDecoder, &g);
        assert_eq!((h.w, h.h), (30, 20));
        assert!(px.chunks_exact(4).all(|p| p[0] == p[1] && p[1] == p[2] && p[3] == 255));
    }
    ...
    impl Decoder for Fake {
        ...
        fn probe(&self, bytes: &[u8]) -> Option<ImageHeader> {
            bytes.starts_with(b"fake").then(|| ImageHeader::new(ColorFormat::Argb8888, self.1, 1))
        }
    ...
    fn registry_probes_in_order() {
        ...
        assert_eq!(r.names().collect::<Vec<_>>(), ["qoi", "png", "jpeg", "bmp", "gif"]);
